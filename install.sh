...
if [ $? -eq 0 ]; then
	...
	docker run -d --restart=unless-stopped --net=wago --name=c_telegraf -v /root/certs/:/etc/telegraf/certs/ -v /root/config/telegraf.conf:/etc/telegraf/telegraf.conf:ro telegraf:1.19.1
